...
function getPublicKeyFromProtoBuf(bytesView, startOffset, endOffset) {
    ...
    function getvarint() {
        ...
        if (bytesView[startOffset++] & 0x80)
            console.warn('proto: not a uint32');
        return val;
    }
    var publicKeys = [];
    var crxIdBin;
    ...
    var crxIdHex = Buffer.from(getBinaryString(crxIdBin, 0, 16), 'binary').toString('hex');
    for (var i = 0; i < publicKeys.length; ++i) {
        var publicKeyBin = Buffer.from(publicKeys[i], 'binary');
        var sha256sum = crypto_1.createHash('sha256').update(publicKeyBin).digest('hex');
...
exports.parseCrx = function (buf) {
    ...
    if (buf[4] === 2) {
        ...
        publicKey = Buffer.from(getBinaryString(buf, 16, 16 + publicKeyLength), 'binary');
    }
    else if (buf[4] === 3) {
        // CRX3 - https://cs.chromium.org/chromium/src/components/crx_file/crx3.proto
        var crx3HeaderLength = calcLength(buf[8], buf[9], buf[10], buf[11]);
    ...
    var crxId = crypto_1.createHash('sha256')
        .update(publicKey)
        .digest('hex')
        .split('')
    ...
    // 16 = Magic number (4), CRX format version (4), lengths (2x4)
    zipStartOffset = 16 + publicKeyLength + signatureLength;
    return { zip: buf.slice(zipStartOffset, buf.length), id: crxId, publicKey: publicKey };
};
