...
function searchEvent(eventName) {
    let searchInput = document.querySelector("#searchFormInput-input");
    if (searchInput) {
...
function clickFirstFindTickets() {
    let ticketsButton = document.querySelector("a[data-testid='event-list-link']");
    if (ticketsButton) {
        ticketsButton.click();
...
function selectFirstTicket() {
    let ticketButton = document.querySelector("button[data-bdd='quick-pick-price-button']");
    if (ticketButton) {
        ticketButton.click();
    
        setTimeout(() => {
            let nextButton = document.querySelector("button[data-bdd='offer-card-buy-button']");
            if (nextButton) {
                nextButton.click();
                
                setTimeout(loginToTicketmaster, 4000);
            } else {
                console.log("not found");
            }
        }, 2000);
...
if (window.location.href == 'https://www.ticketmaster.com/') {
    ...
    setTimeout(loginToTicketmaster, 3000);
} 
